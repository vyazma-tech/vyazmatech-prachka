...
using VyazmaTech.Prachka.Application.Handlers.Core.Queue.Commands.BulkInsertOrders;
...
public sealed class BulkInsertTests : TestBase
{
    private readonly BulkInsertOrdersCommandHandler _handler;
    private readonly Mock<ICurrentUser> _currentUser;

    public BulkInsertTests(CoreDatabaseFixture fixture) : base(fixture)
    {
        var timeProvider = new Mock<IDateTimeProvider>();

        _currentUser = new Mock<ICurrentUser>();
        _handler = new BulkInsertOrdersCommandHandler(
    ...
    public async Task Handle_ShouldThrow_WhenCurrentUserNotAuthenticated()
    {
        ...
        // Act
        var command = new BulkInsertOrders.Command(default, 1);
        Func<Task<BulkInsertOrders.Response>> action = async () =>
    ...
    public async Task Handle_ShouldThrow_WhenQueueNotFound()
    {
        ...
        // Act
        var command = new BulkInsertOrders.Command(default, 0);
        Func<Task<BulkInsertOrders.Response>> action = async () =>
            await _handler.Handle(command, CancellationToken.None);
