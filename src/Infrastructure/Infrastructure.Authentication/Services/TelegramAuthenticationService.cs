...
using Infrastructure.Authentication.Mapping;
...
internal sealed class TelegramAuthenticationService : IAuthenticationService
{
    ...
    public async Task<Result<IdentityTokenDto>> GetUserTokensAsync(string telegramUsername, CancellationToken token)
    {
        ...
        if (searchResult.IsFaulted)
            return new Result<IdentityTokenDto>(searchResult.Error);
    ...
    public async Task<Result<IdentityTokenDto>> RefreshToken(string accessToken, string refreshToken)
    {
        ...
        if (user.RefreshToken != refreshToken || user.RefreshTokenExpiryUtc < DateTime.UtcNow)
            return new Result<IdentityTokenDto>(AuthenticationErrors.IdentityToken.Refresh());
        ...
        CancellationToken token)
    {
        ...
        if (result.Succeeded is false)
            return new Result<IdentityUserDto>(result.ToError());
    ...
    public async Task<Result<IdentityUserDto>> GetUserByIdAsync(Guid userId, CancellationToken token)
    {
        Result<VyazmaTechIdentityUser> searchResult = await _userManager.GetByIdAsync(userId, token);

        if (searchResult.IsFaulted)
            return new Result<IdentityUserDto>(searchResult.Error);

        return searchResult.Value.ToDto();
    }

    public async Task<Result<IdentityUserDto>> GetUserByTelegramUsernameAsync(
